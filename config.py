...
class Config:
    ...
    CACHE_ENCRYPTION_ENABLED = (
        os.getenv("CACHE_ENCRYPTION_ENABLED", "true").lower() == "true"
    )
    CACHE_AUTH_METHOD = os.getenv(
        "CACHE_AUTH_METHOD", "password"
    )  # password, keyfile, yubikey
    ...
    STRUCTURED_PARSING_ENABLED = (
        os.getenv("STRUCTURED_PARSING_ENABLED", "true").lower() == "true"
    )
