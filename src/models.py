...
class ResumeImprovements(BaseModel):
    ...
    keyword_suggestions: List[str] = Field(
        default_factory=list, description="Keywords to add to resume"
    )
    section_improvements: Dict[str, str] = Field(
        default_factory=dict, description="Section-specific improvements"
    )
    ...
    content_enhancements: List[str] = Field(
        default_factory=list, description="Content improvements"
    )
    priority_changes: List[str] = Field(
        default_factory=list, description="Top 3 most important changes"
    )
    quick_wins: List[str] = Field(
        default_factory=list, description="Simple immediate changes"
    )
...
class InterviewQuestions(BaseModel):
    ...
    questions_for_employer: List[str] = Field(
        default_factory=list, description="Questions to ask hiring manager"
    )
    anticipated_questions: List[str] = Field(
        default_factory=list, description="Questions interviewer might ask"
    )
    suggested_answers: List[str] = Field(
        default_factory=list, description="Suggested answers to anticipated questions"
    )
...
class NextSteps(BaseModel):
    ...
    immediate_actions: List[str] = Field(
        default_factory=list, description="Next 1-2 days"
    )
    short_term_preparation: List[str] = Field(
        default_factory=list, description="Next week"
    )
    long_term_development: List[str] = Field(
        default_factory=list, description="Next month"
    )
    application_strategy: List[str] = Field(
        default_factory=list, description="Application approach"
    )
    risk_mitigation: List[str] = Field(
        default_factory=list, description="Challenges and solutions"
    )
...
class ErrorInfo(BaseModel):
    ...
    context: str = Field(
        default="", description="Additional context about where the error occurred"
    )
...
class AnalysisResult(BaseModel):
    ...
    assessment: Optional[JobAssessment] = Field(
        default=None, description="Job assessment results"
    )
    resume_improvements: Optional[ResumeImprovements] = Field(
        default=None, description="Resume improvement suggestions"
    )
    ...
    next_steps: Optional[NextSteps] = Field(
        default=None, description="Action plan and next steps"
    )
    ...
    error_info: Optional[ErrorInfo] = Field(
        default=None, description="Error information if analysis failed"
    )
...
class AnalyticsEvent(BaseModel):
    ...
    event_type: str = Field(description="Type of event (analysis, error, etc.)")
    timestamp: datetime = Field(
