...
class StructuredParser:
    ...
    def _parse_assessment_regex(self, response: str) -> JobAssessment:
        ...
        rating_match = re.search(
            r"(?:rating|score|suitability)[:\s]*(\d+)", response, re.IGNORECASE
        )
        ...
        strengths = self._extract_section_content(
            response, strengths_patterns, "Not specified"
        )
        ...
        missing_requirements = self._extract_section_content(
            response, missing_patterns, "Not specified"
        )
    ...
    def _parse_improvements_regex(self, response: str) -> ResumeImprovements:
        ...
        keywords = [
            kw
            for kw in keywords
            if not kw.lower().endswith(":") and len(kw.strip()) > 2
        ]
        ...
        formatting = [
            ...
            if not fmt.lower().endswith(":") and len(fmt.strip()) > 2
        ]
        ...
        content_enhancements = [
            ce
            for ce in content_enhancements
            if not ce.lower().endswith(":") and len(ce.strip()) > 2
        ]
        ...
        priority_changes = self._extract_list_content(response, priority_patterns)[
            :3
        ]  # Limit to 3
        priority_changes = [
            pc
            for pc in priority_changes
            if not pc.lower().endswith(":") and len(pc.strip()) > 2
        ]
        ...
        quick_wins = [
            ...
            if not qw.lower().endswith(":") and len(qw.strip()) > 2
        ]
    ...
    def _parse_cover_letter_regex(self, response: str) -> CoverLetter:
        ...
        opening = self._extract_section_content(
            response, opening_patterns, "Dear Hiring Manager,"
        )
        ...
        body = self._extract_section_content(
            response, body_patterns, "I am writing to express my interest..."
        )
        ...
        closing = self._extract_section_content(
            response, closing_patterns, "Thank you for your consideration."
        )
        ...
        return CoverLetter(
            opening=opening, body=body, closing=closing, full_letter=full_letter
        )
    ...
    def _parse_interview_questions_regex(self, response: str) -> InterviewQuestions:
        ...
        questions_for_employer = [
            q
            for q in questions_for_employer
            if not q.lower().endswith(":") and len(q.strip()) > 2
        ]
        ...
        anticipated_questions = self._extract_list_content(
            response, anticipated_patterns
        )
        ...
        suggested_answers = [
            a
            for a in suggested_answers
            if not a.lower().endswith(":") and len(a.strip()) > 2
        ]
    ...
    def _parse_next_steps_regex(self, response: str) -> NextSteps:
        ...
        immediate_actions = [
            ia
            for ia in immediate_actions
            if not ia.lower().endswith(":") and len(ia.strip()) > 2
        ]
        ...
        short_term_preparation = self._extract_list_content(
            response, short_term_patterns
        )
        short_term_preparation = [
            st
            for st in short_term_preparation
            if not st.lower().endswith(":") and len(st.strip()) > 2
        ]
        ...
        long_term_development = [
            lt
            for lt in long_term_development
            if not lt.lower().endswith(":") and len(lt.strip()) > 2
        ]
        ...
        application_strategy = [
            as_item
            for as_item in application_strategy
            if not as_item.lower().endswith(":") and len(as_item.strip()) > 2
        ]
        ...
        risk_mitigation = [
            ...
            if not rm.lower().endswith(":") and len(rm.strip()) > 2
        ]
    ...
    def _extract_section_content(
        self, response: str, patterns: list, default: str
    ) -> str:
    ...
    def _extract_recommendation(
        self, response: str, patterns: list
    ) -> RecommendationType:
    ...
    def validate_parsing_success(self, response: str, expected_fields: list) -> bool:
        ...
        if "rating" in expected_fields:
            rating_match = re.search(
                r"(?:rating|score)[:\s]*(\d+)", response, re.IGNORECASE
            )
            if not rating_match:
                return False
        ...
        if "recommendation" in expected_fields:
            rec_match = re.search(
                r"(?:recommendation|should)[:\s]*(yes|no)", response, re.IGNORECASE
            )
            if not rec_match:
                return False
