...
class ResumeCache:
    ...
    def __init__(self, cache_dir: Optional[str] = None, encryption_manager=None):
        if cache_dir is None:
            ...
            self.cache_dir = Path(
                user_cache_dir("job-application-agent", "job-application-agent")
            )
        else:
    ...
    def _load_cache(self) -> dict:
        ...
        if self.cache_file.exists():
            try:
                ...
                console.print(
                    f"[yellow]Warning: Cache file corrupted, starting fresh: {e}[/yellow]"
                )
        return {}
    ...
    def _load_encrypted_cache(self) -> dict:
        ...
        if (
            not self.encryption_manager
            or not self.encryption_manager.is_encryption_enabled()
        ):
            console.print(
                "[red]Error: Encrypted cache found but encryption is disabled[/red]"
            )
            return {}
    ...
    def _save_cache(self):
        ...
        try:
            ...
            if (
                ...
                and self.encryption_manager.is_encryption_enabled()
            ):
    ...
    def get_resume(self, file_path: str) -> Optional[str]:
        ...
        if cache_key in self.cache_data:
            ...
            if cached_entry.get("file_hash") == file_hash:
                console.print("[blue]Using cached resume content[/blue]")
    ...
    def set_resume(self, file_path: str, content: str) -> None:
        ...
        self._save_cache()
        console.print("[green]Cached resume content[/green]")
    ...
    def get_cache_stats(self) -> dict:
        ...
        resume_entries = sum(
            1 for key in self.cache_data.keys() if key.startswith("resume_")
        )
    ...
    def _cleanup_expired_entries(self) -> int:
        ...
        max_age_days = getattr(self.encryption_manager, "config", {}).get(
    ...
    def export_cache(self, output_path: str) -> None:
        ...
        try:
            ...
            export_data = {
                ...
                "total_entries": len(self.cache_data),
                "cache_directory": str(self.cache_dir.absolute()),
                "encryption_enabled": self.encryption_manager.is_encryption_enabled()
    ...
    def get_cache_info(self) -> dict:
        ...
        return {
            ...
            "expiry_days": getattr(self.encryption_manager, "config", {}).get(
                "CACHE_EXPIRY_DAYS", 90
            )
            if self.encryption_manager
            else 90,
    ...
    def display_cache_info(self) -> None:
        ...
        if info["expired_entries"] > 0:
            console.print(
                f"[yellow]Warning: {info['expired_entries']} entries have expired[/yellow]"
            )
        if info["expiring_soon"] > 0:
            console.print(
                f"[yellow]Notice: {info['expiring_soon']} entries will expire soon[/yellow]"
            )
